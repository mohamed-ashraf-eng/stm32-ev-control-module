...
EV\_Update(\&ev, accel\_pct, brake\_pct, 0.01f);
...
The EV state is stored in `EV\_State\_t`, which contains:
...
acceleration = (motor\_torque - drag) / mass\_factor
...
delta\_soc = power × dt / battery\_capacity
...
This makes `regen\_active = 1` and allows SOC recovery behavior to be represented in the simulation.
...
| `Core/Src/ev\_control.c`               | EV model implementation                                 |

| `Core/Inc/ev\_control.h`               | Public EV control API and data structures               |

| `EV\_Control.ioc`                      | STM32CubeIDE / CubeMX configuration                     |

| `Drivers/`                            | STM32 HAL and CMSIS drivers                             |

| `STM32F103C8TX\_FLASH.ld`              | Linker script                                           |
